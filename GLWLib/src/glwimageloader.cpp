...
using glw::util::ImageData;

LDEFINECLASS("GLW", "ImageLoader")
...
void ImageLoader::getData(ImageData& imageData, const char * filename)
{
  int w, h, n;
...
void ImageLoader::getDataPNG(ImageData& imageData, const char * filename)
{
  int w, h, n;
  unsigned char *data = stbi_load(filename, &w, &h, &n, STBI_rgb_alpha);
  loadImageData(imageData, data, w, h, n);
...
void ImageLoader::loadImageData(ImageData& imageData, unsigned char * data, int w, int h, int n)
{
  imageData.w = w;
  imageData.h = h;
  imageData.n = n;
...
//  if (glw::GLWManager::getInstance().isValid())
  {
    glGenTextures(1, &tex);
